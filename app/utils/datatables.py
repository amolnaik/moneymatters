...
from sqlalchemy import func, Text, or_, Numeric
import math
import logging
...
def yadcf_range_date(expr, value):
    v_from, v_to = value.split('-yadcf_delim-')
    #logging.warning("before " + str(v_from) + " " + str(v_to))
    ...
    #logging.warning("after " + str(v_from) + " " + str(v_to))
    return expr.between(v_from, v_to)
...
def yadcf_bool_select(expr, value):
    if value == 'true':
        bvalue = 1
    else:
        bvalue = 0
...
search_methods = {
    ...
    'yadcf_bool_select': yadcf_bool_select, #lambda expr, value: expr == value,
